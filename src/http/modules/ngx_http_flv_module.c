...
ngx_http_flv(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);//找到配置项所属的配置块
    clcf->handler = ngx_http_flv_handler;//http框架在NGX_HTTP_CONTENT阶段时，如果请求的主机名，URI与
                                           //flv配置项所在的配置块匹配，就调用handler处理这个请求
    return NGX_CONF_OK;
}
