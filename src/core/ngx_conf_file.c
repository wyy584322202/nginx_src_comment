...
ngx_conf_set_num_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	...
	/*这样一来np就指向了参数结构体中的偏移量为offset的元素了，类型是ngx_int_t*/
    np = (ngx_int_t *) (p + cmd->offset);

    if (*np != NGX_CONF_UNSET) {
        return "is duplicate";
    }
	/*value指向了一个字符串数组，存储着一个配置项的名字和其参数*/
    value = cf->args->elts;
	/*将ngx_conf_t类型的 cf对象里面的字符串类型的配置项参数转
	 *换成ngx_int_t存储在自定义的用于存储配置项参数的结构体
	 */
    *np = ngx_atoi(value[1].data, value[1].len);
    if (*np == NGX_ERROR) {
        return "invalid number";
    }
	/*post很灵活，自定义解析配置项的方法时其作用完全由自己定，
	 *在使用ngx预设的14个方法的时候这些预设方法各自有其用途
	 *见p121表4-4
	 */
    if (cmd->post) {
        post = cmd->post;
