...
struct ngx_file_s {
    ngx_fd_t                   fd;			//文件描述符
    ngx_str_t                  name;		//文件名
    ngx_file_info_t            info;		//封装的stat,保存文件信息

    off_t                      offset;		//文件偏移量，不用设置
    off_t                      sys_offset;	//文件系统偏移量，不用设置

    ngx_log_t                 *log;			//日志对象，相关的日子会输出到该对象
    ...
    unsigned                   valid_info:1;//未使用的保留字段
    unsigned                   directio:1;	//如果是1就对大文件进行优化，和配置文件的directio配置项对应
};
